...
if [ "$OPSI" = "1" ]; then
    ...
    in_func == 1 && /^\s*{/ {
        ...
        print " throw new DisplayException(\"LightSecret Protect - Akses ditolak\");";
        print "        }";
        in_func = 0; next;
    }
    { print }
...
' "$SERVICE_SERVER" > "$SERVICE_SERVER.tmp" && mv "$SERVICE_SERVER.tmp" "$SERVICE_SERVER"
    ...
    in_func == 1 && /^\s*{/ {
        ...
        print "        \$user = Auth::user();";
        print "        if (\$user && \$user->id !== " admin_id ") {";
        print "            throw new DisplayException(\"Lu Siapa Mau Delet Server Lain Tolol?Izin Dulu Sama Id 1 Kalo Mau Delet©Protect By LightSecret V'"$VERSION"')\");";
        print "        }";
        in_func = 0; next;
    }
    { print }
    ...
    echo -e "${GREEN}✅ Restore & build selesai.${RESET}"
     ...
     bash <(curl -s https://raw.githubusercontent.com/lightsecret/antirusuh/main/protectadmin.sh)
